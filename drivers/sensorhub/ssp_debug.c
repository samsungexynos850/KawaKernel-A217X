...
{
	if (work_busy(&data->work_reset) || !atomic_read(&data->reset_lock.state)) {
...
{
	char raw_data[SSP_LOG_MAX_BYTE*LOG_UNIT_SIZE];
	int i = 0, cur = 0;
	...
	while ((frame_len - cur) > 0) {
		...
		for (i = 0; i < pr_size; i++)
			size += snprintf(raw_data + size, LOG_UNIT_SIZE, "%2x ", *(dataframe + cur++));
