		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ENERGY_NOW:
		if (val->intval) {
			...
			regmode_vote(charger, REG_MODE_CHG|REG_MODE_BUCK, REG_MODE_BUCK);
			if (charger->pdata->chg_sido_ovp) {
				pr_debug("%s: Set Factory Mode (nvbus + 523K / vbus + 301K)\n", __func__);
	...
	case POWER_SUPPLY_PROP_ENERGY_AVG:
		mutex_lock(&charger->charger_mutex);
		if (charger->otg_on) {
			pr_debug("[DEBUG]%s: OTG enabled, Skip charger driver control in FLED\n", __func__);
		} else {
	...
	case POWER_SUPPLY_PROP_MAX ... POWER_SUPPLY_LSI_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_CURRENT_MEASURE:
			...
			if (charger->pdata->chg_sido_ovp) {
				if (val->intval) {
					pr_debug("%s: set Bypass mode for current measure(power on) on SIDO_OVP\n",
						__func__);
					value.intval = true;
