...
u64 kbase_reg_get_gpu_id(struct kbase_device *kbdev)
{
	...
	bv_r49p0_model_read_reg(kbdev->model, GPU_CONTROL_REG(GPU_ID), &val[0]);
...
u32 kbase_reg_read32(struct kbase_device *kbdev, u32 reg_enum)
{
	...
	spin_lock_irqsave(&kbdev->reg_op_lock, flags);
	bv_r49p0_model_read_reg(kbdev->model, offset, &val);
	spin_unlock_irqrestore(&kbdev->reg_op_lock, flags);
...
u64 kbase_reg_read64(struct kbase_device *kbdev, u32 reg_enum)
{
	...
	spin_lock_irqsave(&kbdev->reg_op_lock, flags);
	bv_r49p0_model_read_reg(kbdev->model, offset, &val32[0]);
	bv_r49p0_model_read_reg(kbdev->model, offset + 4, &val32[1]);
	spin_unlock_irqrestore(&kbdev->reg_op_lock, flags);
...
u64 kbase_reg_read64_coherent(struct kbase_device *kbdev, u32 reg_enum)
{
	...
	spin_lock_irqsave(&kbdev->reg_op_lock, flags);
	do {
		bv_r49p0_model_read_reg(kbdev->model, offset + 4, &hi1);
		bv_r49p0_model_read_reg(kbdev->model, offset, &lo);
		bv_r49p0_model_read_reg(kbdev->model, offset + 4, &hi2);
	} while (hi1 != hi2);
	spin_unlock_irqrestore(&kbdev->reg_op_lock, flags);
...
void kbase_reg_write32(struct kbase_device *kbdev, u32 reg_enum, u32 value)
{
	...
	spin_lock_irqsave(&kbdev->reg_op_lock, flags);
	bv_r49p0_model_write_reg(kbdev->model, offset, value);
	spin_unlock_irqrestore(&kbdev->reg_op_lock, flags);
}
...
void kbase_reg_write64(struct kbase_device *kbdev, u32 reg_enum, u64 value)
{
	...
	spin_lock_irqsave(&kbdev->reg_op_lock, flags);
	bv_r49p0_model_write_reg(kbdev->model, offset, value & 0xFFFFFFFF);
	bv_r49p0_model_write_reg(kbdev->model, offset + 4, value >> 32);
	spin_unlock_irqrestore(&kbdev->reg_op_lock, flags);
}
