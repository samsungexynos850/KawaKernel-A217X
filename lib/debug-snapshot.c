...
struct dbg_snapshot_item dss_items[] = {
	...
	{DSS_ITEM_LAST_ID,	DSS_ITEM_LAST,		{0, 0, 0, true, false, true}, NULL ,NULL, 0, },
	...
	for (i = 0; i < ARRAY_SIZE(dss_items); i++) {
		if (dss_items[i].entry.enabled && dss_items[i].entry.paddr && dss_items[i].entry.size) {
			...
			if (!vaddr) {
				...
				dss_items[i].name, (unsigned long)dss_items[i].entry.paddr, (unsigned long)dss_items[i].entry.size);
				return -ENOMEM;
			}
...
{
#ifndef CONFIG_DEBUG_SNAPSHOT_USER_BUILD
	if (dss_dpm.enabled_dump_mode)
...
static void __init dbg_snapshot_boot_cnt(void)
{
	...
	if (dbg_snapshot_get_enable() && item->entry.enabled) {
		...
		dbg_snapshot_hook_logger(NULL, (const char *)_buf, (size_t)strlen(_buf));
	}
}
